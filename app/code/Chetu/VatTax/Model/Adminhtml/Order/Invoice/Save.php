...
namespace Chetu\VatTax\Model\Adminhtml\Order\Invoice;
...
class Save extends \Magento\Sales\Controller\Adminhtml\Order\Invoice\Save
{
    ...
    public function __construct(
        ...
        $this->salesData = $salesData ?: \Magento\Framework\App\ObjectManager::getInstance()->get(SalesData::class);
...
*/
            ...
            /**
             *  When invoice is generating than grand total and base grand total is coming with vat tax so we have
             * removed the vat from the total
             */
            ...
            **/
            ...
            if($this->_helperData->checkStore($orderId)) {
                $invoice->setBaseGrandTotal($invoice->getBaseGrandTotal()-$invoice->getTaxAmount());
                $invoice->setGrandTotal($invoice->getBaseGrandTotal());
            }
            ...
            $transactionSave = $this->_objectManager->create(\Magento\Framework\DB\Transaction::class)->addObject($invoice)->addObject($invoice->getOrder());
            $shipment = false;
